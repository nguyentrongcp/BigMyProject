                ...
                <div class="position-relative">
                    ...
                    <div>
                        <div>
                            ...
                            <div>
                                <span class="fw-bolder">Nhà cung cấp: </span>
                                <span>{{ $phieu->doituong->ten }}</span>
                            </div>
                            <div>
                                <span class="fw-bolder">Điện Thoại: </span>
                                <span>{{ $info->id != '1000000000' ? '' : ($phieu->doituong->dienthoai ?? '') }}</span>
                            </div>
                            <div style="text-align: justify">
                                <span class="fw-bolder">Địa Chỉ: </span>
                                <span>{{ $info->id != '1000000000' ? '' : ($phieu->doituong->diachi ?? '') }}</span>
                            </div>
                            <div>
                            ...
                            <div class="mt-2 d-flex justify-content-end fw-bolder">
                                <div class="w-50">
                                    <div class="d-flex">
                                        ...
                                        <div class="ms-auto">{{ number_format($phieu->giamgia) }}</div>
                                    </div>
                                    <div class="d-flex">
                                        <div>
                                            TỔNG PHỤ THU:
                                        </div>
                                        <div class="ms-auto">{{ number_format($phieu->phuthu) }}</div>
                                    </div>
                                    <div class="d-flex">
                                        <div>
                                            TIỀN THANH TOÁN:
                                        </div>
                                        <div class="ms-auto">{{ number_format($phieu->tienthanhtoan) }}</div>
                                    </div>
                                </div>
                            </div>
                            @endif
                    ...
                    <div class="mt-3">
                        <div class="d-flex justify-content-between">
                            <div class="w-50 text-center">
                                <p class="fw-bolder m-0">Nhà cung cấp</p>
...
<script>
    ...
    @if(!$phieu->trashed())
                ...
                (confirmed) => {
                    if (confirmed.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            if (result.succ) {
                                sToast.notification(1,result.noti,() => {
                                    let optionMessage = {type: 'huy', maphieu: '{{ $phieu->maphieu }}'};
        ...
        @else
        $('#btnXoaPhieu').text('Xóa Phiếu').click(() => {
            sToast.confirm('Xác nhận xóa phiếu nhập hàng?','',
                (confirmed) => {
                    if (confirmed.isConfirmed) {
    ...
    @else
    $('#btnPhucHoi').click(() => {
        sToast.confirm('Xác nhận phục hồi phiếu nhập hàng?','',
            (confirmed) => {
                if (confirmed.isConfirmed) {
